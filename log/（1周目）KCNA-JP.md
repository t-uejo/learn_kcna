...
## サービスメッシュ
- 相互TLSと認証と承認を行う。
- 相互TLSのため、通信相手が証明書を持っているか確認するプロセスがある。
- 承認＝認可のこと。ここでは、通信の内容やリクエストの属性によって、アクセスを許可、拒否するポリシーを設定することができる。

## Podのログ格納先
- Podのログは`/var/log/pods/xxx`に配置されています。

## ヘッドレスサービスとDNSラウンドロビン
- ヘッドレスサービス
	- `clusterIP: None`
	- クライアントAが ‎`nginx.default.svc.cluster.local` を名前解決 → 192.168.76.141, 192.168.76.142 の順で返る → 141にアクセス
	- クライアントBが同じ名前を解決 → 192.168.76.142, 192.168.76.141 の順で返る → 142にアクセス
- 以下の記事が確かにとてもわかりやすかった。
	- https://eng-blog.iij.ad.jp/archives/9998#statefulset%e3%83%aa%e3%82%bd%e3%83%bc%e3%82%b9%e3%81%a8%e3%83%98%e3%83%83%e3%83%89%e3%83%ac%e3%82%b9%e3%82%b5%e3%83%bc%e3%83%93%e3%82%b9

## Stateful Set
- FQDNが「＜Pod名＞.＜Service名＞.＜Namespace名＞.svc.cluster.local」になる。
- 単にPodごとにDNSで名前解決できるホスト名をつけたということ。
